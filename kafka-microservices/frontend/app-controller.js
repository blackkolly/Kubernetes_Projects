...
window.onload = function() {
    ...
    window.addToLog = function(msg) {
      ...
      if (logElem) logElem.innerHTML = logList.map(m => `<li>${m}</li>`).join('');
    ...
    function updateParamInputs() {
      ...
      weatherLat.style.display = weatherLon.style.display = dataSource.value === 'weather' ? '' : 'none';
    }
    if (dataSource) {
    ...
    startBtn.onclick = function() {
      ...
      fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
        .then(res => res.json())
        .then(data => addToLog('Started streaming: ' + dataSource.value + ' | Response: ' + JSON.stringify(data)))
    ...
    stopBtn.onclick = function() {
      ...
      fetch(endpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
        .then(res => res.json())
        .then(data => addToLog('Stopped streaming: ' + dataSource.value + ' | Response: ' + JSON.stringify(data)))
        .catch(err => addToLog('Error: ' + err.message));
    };
        ...
        if (['news', 'air'].includes(dataSource.value)) {
          startBtn.onclick = function() {
            ...
            fetch(endpoint, { method: 'POST' })
              .then(res => res.json())
              .then(data => addToLog('Sent ' + dataSource.value + ' data | Response: ' + JSON.stringify(data)))
              .catch(err => addToLog('Error: ' + err.message));
          };
          stopBtn.onclick = function() { addToLog('No continuous streaming for this source.'); };
        } else {
          startBtn.onclick = function() {
            if (streaming) return;
            streaming = true;
    ...
    const consumedDataContainer = document.getElementById('consumedDataContainer');
    ...
    function renderConsumedData(data) {
      ...
      consumedDataList.innerHTML = data.map(item => {
        let ts = item.timestamp ? new Date(item.timestamp).toLocaleString() : '';
        return `<li><b>${ts}</b>: <pre style="display:inline; color:#fff;">${JSON.stringify(item, null, 2)}</pre></li>`;
      }).join('');
    }
    if (fetchConsumedBtn) {
      fetchConsumedBtn.onclick = function() {
        ...
        fetchConsumedBtn.textContent = consumedVisible ? 'Hide Consumed Data' : 'Fetch Consumed Data';
        if (consumedVisible) {
